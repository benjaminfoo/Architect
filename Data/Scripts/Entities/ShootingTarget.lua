...
ShootingTarget.Server.Deactivated = {
    OnBeginState = function(self)
        if (self.init == 0) then
            System.LogAlways("Loading deactivated - onbeginstate ")
            local props = self.Properties;
            self:KillTimer(ACTIVATION_TIMER);
            self:KillTimer(TURN_TIMER);
            AI.SetSmartObjectState(self.id, "Neutral");
            if (props.fIntervallMin > props.fIntervallMax) then
                props.fIntervallMin = props.fIntervallMax;
            end ;
    ...
    OnTimer = function(self, timerId, msec)
        System.LogAlways("Loading deactivated - onTimer ")
    ...
    OnContact = function(self, collider)
        System.LogAlways("OnContact omg! ")

        if (collider.driverT and self.bBreakByCar == 1) then
            self:GoDead();
        end
    end,
    OnDamage = function(self, collider)
        System.LogAlways("OnDamage omg! ")
    end,
    OnCollide = function(self, collider)
        System.LogAlways("OnCollide omg! ")
    end,
};
...
ShootingTarget.Server.Activated = {
    OnBeginState = function(self)
        self.ended = 0;
        System.LogAlways("Loading activated - onbeginstate ")
